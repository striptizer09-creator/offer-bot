...
# ==================== ПОЛУЧАЕМ ТОКЕН ИЗ НАСТРОЕК KOYEB ====================
# Важно: Токен берется из настроек Koyeb, а не из кода!
API_TOKEN = os.getenv("BOT_TOKEN")
if not API_TOKEN:
    logger.error("❌ ОШИБКА: BOT_TOKEN не найден!")
    logger.error("Добавьте BOT_TOKEN в настройках Koyeb:")
    logger.error("Settings → Environment Variables → Add Variable")
    sys.exit(1)
...
async def cmd_start(message: types.Message):
    ...
    welcome_text = (
        "Привет, пришли что-то свое и не забудь указать автора (желательно ссылку на него). Если не указан автор, предложение не принимается."
    )
...
async def on_startup():
    ...
    logger.info("=" * 50)
    logger.info("🚀 БОТ-ПРЕДЛОЖКА ЗАПУСКАЕТСЯ НА KOYEB")
    logger.info("=" * 50)
